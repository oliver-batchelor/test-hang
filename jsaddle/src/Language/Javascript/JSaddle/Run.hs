...
import Control.Monad (void, when, zipWithM_, forever)
...
runJavaScript sendBatch entryPoint = do
    ...
    let putStrLn str = return ()-- putMVar logVar str
    ...
    _ <- forkIO . numberForeverFromM_ 1 $ \nBatch -> do
        putStrLn (show ("before readBatch", nBatch))
        (batch@(Batch cmds _ _), resultMVars) <- readBatch nBatch commandChan 
                
        logInfo (\x -> "Sync " <> x <> show (length cmds, last cmds))
        _ <- tryTakeMVar lastAsyncBatch
        putMVar lastAsyncBatch batch
        putStrLn "before sendBatch"
        sendBatch batch
        putStrLn "after sendBatch"
  ...
  where
    ...
    takeResult recvMVar nBatch = do
        result <- takeMVar recvMVar 
        putStrLn (show ("takeresult", nBatch))
